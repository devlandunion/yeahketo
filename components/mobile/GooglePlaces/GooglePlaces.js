...
function GoogleSuggest({meta, onChange, children, changePlace, labelName}) {
  ...
  return (
    <React.Fragment>
      ...
      <div className="desktop-field address-line-1">
        {children}
        <GoogleMapLoader
          params={{
            key: MY_API_KEY,
          ...
          render={googleMaps =>
            googleMaps && (
              ...
              >
                <label
                  className={`address-line-1-label ${meta.touched &&
...
function autoComplete(item) {
  ...
  if (firstMatchedString) {
    labelParts = {
      before: structured_formatting.main_text.substr(
        0,
      ...
      secondary_text: structured_formatting.secondary_text.substring(
        0,
        cutLength - structured_formatting.main_text.length
      )
    }
  }
